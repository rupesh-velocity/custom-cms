...
function MobileMenuNode({
  ...
  const isSub = depth > 0;
  const textStyle = isSub
    ? "text-base text-gray-600 font-normal hover:text-[#5e3fde]"
    : "text-lg font-medium text-gray-800 hover:text-[#5e3fde]";
            ...
            {isAuthenticated ? (
              <div className="mt-4 pt-4 border-t border-gray-100 flex flex-col gap-1">
                ...
                <form action="/api/users/logout" method="POST">
                  <button
                    type="submit"
                    className="text-lg font-medium text-red-500 hover:text-red-700 py-2 text-left w-full"
                  >
                    Logout
                  </button>
                </form>
