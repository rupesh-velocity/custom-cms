...
export default async function SiteHeader({
  ...
  if (
    ...
    settings.homepage_page_id
  ) {
    const hpPage = await prisma.page.findUnique({
      where: { id: parseInt(settings.homepage_page_id) },
    });
  ...
  const menuTree = primaryMenu
    ? buildTree(primaryMenu.items, null, homepageSlug)
    : [];
  ...
  return (
    <header className="site-header h-auto">
      <div className="container py-4 lg:h-full flex flex-wrap items-center justify-between">
        <Link
          href="/"
          className="shrink-0 flex items-center w-auto max-w-[70%]"
        >
          {siteLogo ? (
            <img
              src={optimizeLogoUrl(siteLogo)}
              alt={siteTitle}
        ...
        <div className="flex items-center gap-4 lg:gap-10">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {isAuthenticated ? (
              <div className="relative group flex items-center cursor-pointer">
                ...
                <div className="absolute top-full right-0 hidden group-hover:block min-w-[160px] bg-white border border-gray-100 shadow-xl rounded-lg py-2 z-50">
                  <Link
                    ...
                    className="block px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-[#5e3fde]"
                  >
                    Dashboard
                  </Link>
                  <form
              ...
              <Link
                href="/login"
                className="flex items-center gap-2 text-sm font-semibold text-gray-700 hover:text-[#5e3fde]"
              >
                <LogIn size={16} /> Login
              </Link>
            )}
            ...
            {!isCustomerOrSubscriber && (
              <Link
                href="https://custom-cms-three.vercel.app/on-demand/"
                className="theme-btn theme-btn-blue ml-2"
              >
                On Demand Classes{" "}
                <span>
                  <img
                    alt=""
                    src="https://res.cloudinary.com/cgibjxmm/image/upload/f_auto,q_auto/v1785921454/custom-cms/blue-button-arrow.png"
                  />
                </span>
              </Link>
            )}
